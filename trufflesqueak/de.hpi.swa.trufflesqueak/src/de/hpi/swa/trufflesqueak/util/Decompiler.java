...
public class Decompiler {
    ...
    public static SqueakNode[] blockFrom(List<SqueakBytecodeNode> sequence, Stack<SqueakNode> stack) {
        ...
        while (i < sequence.size()) {
            ...
            if (node != null) {
                ...
                if (!stack.empty() && statements.size() > size) {
                    // some statement was discovered, but something remains on
                    // the stack. this means the statement didn't consume
                    // everything from under itself, and we need to insert the
                    // stack item before the last statement.
                    // FIXME: can these asserts be wrong?
                    assert statements.size() == size + 1;
                    assert stack.size() == 1;
                    // statements.insertElementAt(stack.pop(), size);
                }
            } else {
                i += 1;
            }
    ...
    private int nextByte(int[] indexRef) {
        ...
        if (getBytes().length <= index) { return 0; }
