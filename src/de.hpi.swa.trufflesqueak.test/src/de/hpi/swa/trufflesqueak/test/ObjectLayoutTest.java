...
public final class ObjectLayoutTest extends AbstractSqueakTestCaseWithDummyImage {
    ...
    public void testBasic() {
        ...
        for (int i = 0; i < obj1.getNumSlots(); i++) {
            assertSame("All reads should return nil", readNode.execute(readNode, obj1, i), NilObject.SINGLETON);
        }

        /* Ensure nil writes do not change uninitialized locations. */
        for (int i = 0; i < obj1.getNumSlots(); i++) {
            writeNode.executeNil(writeNode, obj1, i);
        }
        for (final SlotLocation location : obj1.getLayout().getLocations()) {
        ...
        assertSame(readNode.execute(readNode, obj2, 1), NilObject.SINGLETON);
        ...
        assertEquals(1234L, readNode.executeLong(readNode, obj2, 12));
    ...
    private static PointersObject instantiate(final ClassObject dummyClass) {
        return (PointersObject) SqueakObjectNewNode.getUncached().execute(SqueakObjectNewNode.getUncached(), image, dummyClass);
    }

    private static void writeAndValidate(final AbstractPointersObject obj, final int index, final Object value) {
        AbstractPointersObjectWriteNode.getUncached().execute(AbstractPointersObjectWriteNode.getUncached(), obj, index, value);
        assertEquals("Write failed", AbstractPointersObjectReadNode.getUncached().execute(AbstractPointersObjectReadNode.getUncached(), obj, index), value);
    }
}
