...
public final class SqueakImageContext {
    ...
    public void ensureLoaded() {
        if (!loaded()) {
            ...
            getActiveProcessSlow().atput0Slow(PROCESS.SUSPENDED_CONTEXT, NilObject.SINGLETON);
    ...
    public ExecuteTopLevelContextNode getActiveContextNode() {
        ...
        final ContextObject activeContext = (ContextObject) activeProcess.at0Slow(PROCESS.SUSPENDED_CONTEXT);
        activeProcess.atput0Slow(PROCESS.SUSPENDED_CONTEXT, NilObject.SINGLETON);
    ...
    public ClassObject getWideStringClass() {
        if (wideStringClass == null) {
            ...
            if (method != null) {
                ...
                wideStringClass = (ClassObject) assoc.at0Slow(ASSOCIATION.VALUE);
            } else {
    ...
    public PointersObject getScheduler() {
        if (scheduler == null) {
            CompilerDirectives.transferToInterpreterAndInvalidate();
            scheduler = (PointersObject) schedulerAssociation.at0Slow(ASSOCIATION.VALUE);
        }
        return scheduler;
    }
    ...
    public Object getGlobals() {
        final PointersObject environment = (PointersObject) smalltalk.at0Slow(SMALLTALK_IMAGE.GLOBALS);
        final PointersObject bindings = (PointersObject) environment.at0Slow(ENVIRONMENT.BINDINGS);
