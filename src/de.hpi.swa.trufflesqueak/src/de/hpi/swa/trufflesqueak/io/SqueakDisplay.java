...
public final class SqueakDisplay {
    ...
    private static final class SqueakDisplayCanvas extends Component {
        ...
        private void setSqueakDisplay(final PointersObject squeakDisplay) {
            ...
            final NativeObject bitmap = readNode.executeNative(readNode, squeakDisplay, FORM.BITS);
            if (!bitmap.isIntType()) {
                throw SqueakException.create("Display bitmap expected to be a words object");
            }
            final int width = readNode.executeInt(readNode, squeakDisplay, FORM.WIDTH);
            final int height = readNode.executeInt(readNode, squeakDisplay, FORM.HEIGHT);
