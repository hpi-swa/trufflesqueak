...
public final class ExecuteContextNode extends AbstractExecuteContextNode {
    ...
    @Override
    public Object executeFresh(final VirtualFrame frame, final int pc) {
        try {
    ...
    @Override
    public Object executeResume(final VirtualFrame frame, final int pc) {
        try {
            return resumeBytecode(frame, pc);
        } catch (final NonLocalReturn nlr) {
            /** {@link getHandleNonLocalReturnNode()} acts as {@link BranchProfile} */
            return getHandleNonLocalReturnNode().executeHandle(frame, nlr);
        } finally {
    ...
    private Object startBytecode(final VirtualFrame frame, final int firstPC) {
        ...
        bytecode_loop: while (pc != LOCAL_RETURN_PC) {
            ...
            if (node instanceof CallPrimitiveNode) {
                ...
                assert pc == firstPC + CallPrimitiveNode.NUM_BYTECODES;
                continue;
