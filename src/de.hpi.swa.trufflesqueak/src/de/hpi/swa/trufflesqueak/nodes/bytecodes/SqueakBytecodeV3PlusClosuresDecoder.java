    ...
    public AbstractBytecodeNode decodeBytecode(final VirtualFrame frame, final CompiledCodeObject code, final AbstractBytecodeNode[] bytecodeNodes, final int index) {
        ...
        return switch (b) {
            case 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F //
                -> PushBytecodes.PushReceiverVariableNode.create(index + 1, b & 15);
            case 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F //
                -> new PushBytecodes.PushTemporaryLocationNode(index + 1, b & 15);
            ...
            case 0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67 -> new StoreBytecodes.PopIntoReceiverVariableNode(index + 1, b & 7);
            case 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F -> new StoreBytecodes.PopIntoTemporaryLocationNode(index + 1, b & 7);
            case 0x70 -> new PushBytecodes.PushReceiverNode(index + 1);
            case 0x71 -> new PushBytecodes.PushConstantTrueNode(index + 1);
            case 0x72 -> new PushBytecodes.PushConstantFalseNode(index + 1);
            case 0x73 -> new PushBytecodes.PushConstantNilNode(index + 1);
            case 0x74 -> new PushBytecodes.PushConstantMinusOneNode(index + 1);
            ...
            case 0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F -> JumpBytecodes.ConditionalJumpOnFalseNode.createShort(index + 1, b);
            case 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7 -> JumpBytecodes.createUnconditionalLongJump(bytecodeNodes, index, b, bytecode[index + 1]);
            case 0xA8, 0xA9, 0xAA, 0xAB -> JumpBytecodes.ConditionalJumpOnTrueNode.createLong(index + 2, b, bytecode[index + 1]);
            case 0xAC, 0xAD, 0xAE, 0xAF -> JumpBytecodes.ConditionalJumpOnFalseNode.createLong(index + 2, b, bytecode[index + 1]);
            case 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7, 0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2, 0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE, 0xCF  //
                -> SendBytecodes.SendSpecialNode.create(frame, index + 1, b - 176);
