    ...
    public AbstractBytecodeNode decodeBytecode(final VirtualFrame frame, final CompiledCodeObject code, final AbstractBytecodeNode[] bytecodeNodes, final int index, final int sp) {
        ...
        return switch (b) {
            ...
            case 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F //
                -> new PushBytecodes.PushTemporaryLocationNode(index + 1, sp + 1, b & 15);
            ...
            case 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x5B, 0x5C, 0x5D, 0x5E, 0x5F //
                -> PushBytecodes.PushLiteralVariableNode.create(code, index + 1, sp + 1, b & 31);
            case 0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67 -> new StoreBytecodes.PopIntoReceiverVariableNode(index + 1, sp - 1, b & 7);
            case 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F -> new StoreBytecodes.PopIntoTemporaryLocationNode(index + 1, sp - 1, b & 7);
            ...
            case 0x7D -> new ReturnBytecodes.ReturnTopFromBlockNode(sp);
            ...
            case 0x81 -> MiscellaneousBytecodes.ExtendedBytecodes.createStoreInto(code, index + 2, sp, bytecode[index + 1]);
            case 0x82 -> MiscellaneousBytecodes.ExtendedBytecodes.createPopInto(code, index + 2, sp, bytecode[index + 1]);
            case 0x83 -> {
            ...
            case 0x87 -> new MiscellaneousBytecodes.PopNode(index + 1, sp - 1);
            case 0x88 -> new MiscellaneousBytecodes.DupNode(index + 1, sp + 1);
            ...
            case 0x8B -> MiscellaneousBytecodes.CallPrimitiveNode.create(code, index + 3, sp, bytecode[index + 1], bytecode[index + 2]);
            case 0x8C -> PushBytecodes.PushRemoteTempNode.create(index + 3, sp + 1, bytecode[index + 1], bytecode[index + 2]);
            case 0x8D -> new StoreBytecodes.StoreIntoRemoteTempNode(index + 3, sp, bytecode[index + 1], bytecode[index + 2]);
