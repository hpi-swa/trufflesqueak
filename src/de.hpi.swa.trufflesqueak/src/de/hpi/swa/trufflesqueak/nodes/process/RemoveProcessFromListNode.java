...
public abstract class RemoveProcessFromListNode extends AbstractNode {
    ...
    public final void executeRemove(final Node node, final AbstractPointersObjectReadNode readNode, final AbstractPointersObjectWriteNode writeNode, final PointersObject process,
                    final PointersObject list) {
        final Object first = readNode.execute(node, list, LINKED_LIST.FIRST_LINK);
        final Object last = readNode.execute(node, list, LINKED_LIST.LAST_LINK);
        executeRemove(node, process, list, first, last);
        writeNode.executeNil(node, process, PROCESS.NEXT_LINK);
    }
