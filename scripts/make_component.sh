...
readonly GIT_UNIX_TIMESTAMP=$(git log -1 --format="%ct")
...
echo "COMMIT_INFO={\"${GIT_BRANCH_NAME}\": {\"commit.committer\": \"${GIT_COMMITTER_NAME} <${GIT_COMMITTER_EMAIL}>\", \"commit.committer-ts\": ${GIT_UNIX_TIMESTAMP}, \"commit.rev\": \"${GIT_HASH}\"}}" >> "${RELEASE_FILE}"
